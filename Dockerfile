...
RUN cd /tmp && wget ftp://ftp.freeradius.org/pub/freeradius/freeradius-server-3.0.15.tar.gz && tar -xzvf freeradius-server-3.0.15.tar.gz && cd freeradius-server-3.0.15 && ./configure && make && make install
